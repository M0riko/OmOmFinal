...
export function AuthModal({ open, onOpenChange }: AuthModalProps) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>{t("signIn")}/{" "}Реєстрація</DialogTitle>
          <DialogDescription>Demo auth</DialogDescription>
        </DialogHeader>
        <Tabs defaultValue="login">
          ...
          <TabsContent value="login" className="space-y-3">
            ...
            <Button disabled={isLoading || !email} className="w-full" onClick={handleLogin}>
              {isLoading ? "..." : t("signIn")}
            </Button>
          </TabsContent>
          <TabsContent value="register" className="space-y-3">
            ...
            >
              ...
              {isLoading ? "..." : "Зареєструватися через Google"}
            </Button>
            <div className="relative">
              <div className="absolute inset-0 flex items-center">
                <span className="w-full border-t" />
            ...
            <Input placeholder="Email" type="email" value={email} onChange={(e) => setEmail(e.target.value)} />
            <Input placeholder="Пароль" type="password" value={password} onChange={(e) => setPassword(e.target.value)} />
            <Button disabled={isLoading || !email} className="w-full" onClick={handleRegister}>
